...
| 7b | `kernel_fit_ensemble_v1.npz` (`resp_fitcov_diag`) | yes | NOT YET → accepted as hash-pinned (builder committed, embedded provenance `70efc09`) | rebuild not attempted; pre-`0ecfeea` ancestry disclosed |
...
## 2. Test matrix of record (filled from the runs at HEAD; see the receipt for job ids)
| profile | env | status | notes |
|---|---|---|---|
| hbi (55 files) | gpdla-hbi | see receipt | run on a compute node (`run_test_profile.sbatch`); incl. the two dispositioned cache contracts (znz tolerance PASS; r_emp strict XFAIL) |
| finder (46 files) | gpdla | see receipt | `--continue-on-collection-errors`; genuine finder failures are returned to the PI, not fixed silently |
| training (15 files) | gpdla | see receipt | |
| legacy (2 files) | — | documented as not runnable (pre-FILTER-refactor API) | `tests/profiles/README.md` |
