...
| 7.4 | `adopted_response_v1p1.npz` | yes | builder RECOVERED verbatim and committed (`CDDF_analysis/hbi/adopted_response/`, 4f5e7cc); regeneration + comparison: SLURM 58804619 (receipt) | `build_adopted_response.py --work-dir … --out … --compare …` |
| 7.5 | pack variants | yes | current-contract `resp_fitcov_diag` pack: guards PASS, only `resp_fitcov_diag` differs (1.3e-8); posterior impact: s22 chain on the variant pack (receipt) | `extract_pack_real.py --stamp-v12 --kfe … / --adopted …` |
