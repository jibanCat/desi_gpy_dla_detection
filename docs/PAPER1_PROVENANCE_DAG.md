...
All: GP model (1), `combine_dlacat.py` + `add_dla_flags.py`; truth from quickquasars/`lyatools-make-dla-cat` (`--seed 0` both; toolchain versions NOT recorded; not in the manifest). Class **ARCHIVED-HASH-PINNED** for the tag (numerically re-runnable in principle: mocks + model on Turbo, envs committed). The 2LPT-0 finder predates the default-off clustering hook in `dla_gp.py`; **measured equivalent** to HEAD with `PAIR_PRIOR_MODE=off` on one healpix (matrix row 5). Finder run-to-run: numerically reproducible only — the multi-DLA SIR resampling is unseeded (matrix row 5′).
...
| 7.3 kernel-fit ensemble | `python CDDF_analysis/hbi_mcmc/build_kernel_fit_ensemble.py --n-draws 400 --seed 20260817 [--frozen-npz …] --out …` (run BY FILE PATH in gpdla; `--out` is required) | `475c62b` (file identical to HEAD; `code_commit 70efc09` stamped) | 7.2 output + calibration events | seed 20260817; unit-weight gate | `mu/sig/skew_coef (400,3,3,3)`, `point_*`, `n_events`, `n_uniq_tids`, `seed`, `provenance` | T — see matrix §7 |
| 7.4 adopted operator | `CDDF_analysis/hbi/adopted_response/build_adopted_response.py` (the 2026-08-16 chain recovered verbatim; runbook §6a) | `74762e9` (v1) → `933b0bb` (v1.1, hierarchical) | 73,845 tilt-host pairs (op-cut, x̂ ≥ 19.5, censored anchors self-excluded) | seed0 20260818; 96-draw multinomial sightline bootstrap; LOGO 15/15 | `mu/sig/skew_coef (3,3,4)`, `fit_rng (3,3,2)`, `N_ref`, `carrier_* (96,3,3,4)`, `carrier_shared3 (96,3)`, `carrier_unit_gate (3,)`, `provenance` | T — **BITWISE arrays** (regeneration 2026-08-26; pack identical) |
