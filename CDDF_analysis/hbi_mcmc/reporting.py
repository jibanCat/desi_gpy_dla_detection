...
GATE_AUTHORITY = {
    ...
    "ratio_span_by_z_max": _unratified(
        ...
        # RESOLVED AT MERGE (2026-08-05). This read True on the adopted-basis
        # branch, and that was HONEST there: a behavioural probe measured 7/7
        # tolerances contributing to pass/fail, so writing False would have been
        # the same fabrication in the other direction. The gate stream has since
        # DISARMED the span arms per the PI direction, so the record is now
        # updated to match measured behaviour rather than the other way round.
        # test_declared_gating_matches_what_the_gate_MEASURABLY_does is what
        # forced this line to change, in exactly the way it was designed to.
        ...
        note="0.10 was chosen by eye on 2026-07-29 as 'a swing a sampler "
             ...
             "rate. Disarmed on the integrated tree per the PI direction of "
             "2026-08-05: exceeding it produces an ADVISORY, never a refusal. "
             "Do NOT read 'the span arm did not fire' as 'the span arm "
             "passed' -- no threshold was applied."),
    "ratio_span_by_snr_max": _unratified(
        ...
        note="Same provenance as ratio_span_by_z_max; the wider value only "
             ...
             "prospective calibration is for. Disarmed at the same time and "
             "for the same reason; see ratio_span_by_z_max."),
}
...
def _ratification_module():
    """Load ``CDDF_analysis.hbi_mcmc.ratification`` file-directly.

    Same trick as ``extract_pack._reporting_module`` and ``pack.py``: the
    ``hbi_mcmc`` package ``__init__`` does ``import jax``, so the ordinary
    import path is unavailable in the jax-free ``gpdla`` env that runs
    ``extract_pack``.  ``ratification.py``'s own content is jax-free (measured),
    so loading it by path works in BOTH envs.
    """
    import importlib.util
    import os
    import sys
    mod = sys.modules.get("_modelA_ratification_nojax")
    if mod is None:
        p = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "ratification.py")
        spec = importlib.util.spec_from_file_location(
            "_modelA_ratification_nojax", p)
        mod = importlib.util.module_from_spec(spec)
        sys.modules["_modelA_ratification_nojax"] = mod
        spec.loader.exec_module(mod)
    return mod


def _status_from_ratification(name):
    """``name``'s authority state, READ FROM THE SINGLE SOURCE.

    🔴 MERGE (2026-08-05).  This module used to answer from its own
    ``GATE_AUTHORITY`` table, which carried its own ``PI_RATIFIED_ITEMS``, its
    own verbatim decision text, its own guard and its own stamp -- a COMPLETE
    PARALLEL authority module.  Two tables sharing one vocabulary is precisely
    how a ratification claim drifts back out of sync, and the tree scanner
    flagged every such accessor (``R7_UNDERIVED``).  The records below keep
    their local prose, provenance and notes; the STATUS -- the only part that
    carries authority -- now comes from
    ``CDDF_analysis.hbi_mcmc.ratification``, and
    ``_assert_authority_agrees_with_ratification`` refuses the import if the
    two ever disagree.
    """
    return _ratification_module().record(name).get("status")


def _assert_authority_agrees_with_ratification():
    """Fail CLOSED at import if the local records contradict the single source."""
    RAT = _ratification_module()
    drift = []
    for name, rec in sorted(GATE_AUTHORITY.items()):
        local = rec.get("status")
        canon = RAT.record(name).get("status")
        if local != canon:
            drift.append(f"{name}: GATE_AUTHORITY says {local!r}, "
                         f"ratification.py says {canon!r}")
        lg = bool(rec.get("contributes_to_pass_fail"))
        cg = bool(RAT.gates(name))
        if lg != cg:
            drift.append(f"{name}: GATE_AUTHORITY says gates={lg}, "
                         f"ratification.py says gates={cg}")
    if drift:
        raise FabricatedAuthorityError(
            "reporting.GATE_AUTHORITY has drifted from the single source "
            "CDDF_analysis.hbi_mcmc.ratification:\n  " + "\n  ".join(drift) +
            "\nThe authority state lives in ONE place. Fix the record there, "
            "not here.")
    return True
...
def ratified_gate_tolerances():
    ...
    DERIVED FROM ``CDDF_analysis.hbi_mcmc.ratification``, not from a local
    literal: a list of names typed next to the thing it describes is exactly
    how the fabricated claim survived.
    """
    return tuple(sorted(k for k in GATE_AUTHORITY
                        if _status_from_ratification(k) == RATIFIED))


def restated_not_ratified_gate_tolerances():
    """Gate tolerance names that GATE with no ratified authority (the |z| arms).

    DERIVED FROM ``CDDF_analysis.hbi_mcmc.ratification``.
    """
    return tuple(sorted(k for k in GATE_AUTHORITY
                        if _status_from_ratification(k)
...
def unratified_gate_tolerances():
    ...
    """
    return tuple(sorted(k for k in GATE_AUTHORITY
                        if _status_from_ratification(k) == UNRATIFIED))
...
def gate_authority_stamp():
    ...
    return {
        ...
        # derived from CDDF_analysis.hbi_mcmc.ratification (see
        # _status_from_ratification); spelled out here so the claim and
        # its provenance are visible at the same place.
        "ratified": [n for n in GATE_AUTHORITY
                     if _status_from_ratification(n) == RATIFIED],
...
# ... and refuse the import if this module's records have drifted from the ONE
# source. The allow-list guard above catches a FABRICATED claim; this catches a
# SILENT DIVERGENCE, which is the failure mode two parallel tables actually have.
_assert_authority_agrees_with_ratification()
