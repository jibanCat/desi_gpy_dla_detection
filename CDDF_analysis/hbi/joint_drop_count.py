...
def drop_tau_model(
    ...
    if logN_grid is None:
        # cover the drop kernel (peaks ~16.5-18.5) up through the DLA tail; extend below the counting
        # floor so the sub-LLS opacity is explicit (review's form-bridge fix)
        logN_grid = np.linspace(15.0, 22.5, 160)
...
class DropData:
    ...
    def __init__(self, z912, tau_hat, sigma, z_qso, sigma912=SIGMA_912, beta=3.0):
...
def drop_neg_loglike(theta, family, cfg, drop: DropData):
    tau_m = drop_tau_model(theta, family, cfg, drop.z912, drop.z_qso,
                           sigma912=drop.sigma912, beta=drop.beta)
...
# ---------------------------------------------------------------------------
def fit_joint(fwd, family, cfg, drop: DropData | None, n_restart: int = 8, seed: int = 0):
    ...
    def obj(th):
        return joint_neg_logP(th, fwd, family, cfg, drop)
