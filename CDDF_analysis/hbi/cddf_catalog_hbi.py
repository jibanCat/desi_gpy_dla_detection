...
def build_cnz_resolved(cfg, cat_cut, truth_cut, good_mask, mm):
    ...
    from CDDF_analysis.hbi.znz_kernel import measure_c_nz, CNZModel
    z_edges_fine = _fine_z_grid(cfg)
    n_zf = len(z_edges_fine) - 1
...
# =============================================================================
# The earlier v3 block (gamma/dpl/schechter_z) was REMOVED as dead code (2026-06-23):
# it was SUPERSEDED by this v3.x section and its sole entry point (v3_refit) was never
# called by any live path or test. The 4-lens design review (2026-06-13) found that
# removed amplitude parameterization mis-scaled by ~18 dex (the cold start gave
# f(20.3)=9e-42 vs truth 1.8e-22 and the bound (-26,-18) clamped the optimizer away
# from truth — verified empirically), and its closure target / R0 handling, family
...
def v3x_build_forward(cfg, cat_cut, good_mask, mm, qso_per_sl, logN_lo, logN_hi,
    ...
    if kappa2d is not None:
        ...
        if _znz_path is not None:
            from CDDF_analysis.hbi.znz_kernel import apply_znz_correction, load_znz
    ...
    if getattr(cfg, "completeness_z_resolved", False) and _cnz_resolved is not None:
        ...
        from CDDF_analysis.hbi.znz_kernel import load_znz
