...
def format_latex_num(number, prec=3, trans=-3):
    ...
    if 1 >= exponent > trans:
        ...
        return str("$ {0:.2f} \\times 10^{{ {1:d} }}$").format(
...
def format_latex_two_num(number, number2, prec=3, trans=-3):
    ...
    if 1 >= exponent > trans:
        ...
        return str("$ [{0:.2f}  - {1:.2f} ]\\times 10^{{ {2:d} }}$").format(
            number / 10**exponent, number2 / 10**exponent, exponent
        )
...
def load_cddf_table(txtname, caption=""):
    ...
    table_string += (
        "$\log_{10} \mathrm{N}_\mathrm{HI}$ & $f(N_\mathrm{HI})$ " + scalestr
    )
    ...
    xerr = (table[1, 0] - table[0, 0]) / 2.0
    for row in table:
        if row[1] == row[3] == row[5] == 0.0:
            break
        # Print first two
        table_string += format_latex_two_num(row[0] - xerr, row[0] + xerr, prec=1)
        table_string += " & " + format_latex_num(row[1] / scalefact, trans=-2)
        # Now print errors
        table_string += " & " + format_latex_two_num(
            row[2] / scalefact, row[3] / scalefact, trans=-2
        )
        table_string += " & " + format_latex_two_num(
            row[4] / scalefact, row[5] / scalefact, trans=-2
        )
        table_string += "  \\\\ \n"
    table_string += "\\hline \n  \\end{tabular}\n "
    table_string += "\\caption{" + caption + "}\n"
...
def print_all_tables():
    ...
    print(
        load_table(
            "DR12/dndx_all.txt",
            colheaders=("$z$", "dN/dX"),
            caption="Table of dN/dX values",
        )
    )
    print(
        load_table(
            "DR12/omega_dla_all.txt",
            colheaders=("$z$", "$\Omega_\mathrm{DLA} (10^{-3}) $"),
            caption="$\Omega_\mathrm{DLA}$ values",
            omega=True,
        )
    )
...
def print_all_multi_dlas_tables(path):
    ...
    print(
        load_table(
            os.path.join(path, "dndx_all.txt"),
            colheaders=("$z$", "dN/dX"),
            caption="Table of dN/dX values",
        )
    )
    print(
        load_table(
            os.path.join(path, "omega_dla_all.txt"),
            colheaders=("$z$", "$\Omega_\mathrm{DLA} (10^{-3}) $"),
            caption="$\Omega_\mathrm{DLA}$ values",
            omega=True,
        )
