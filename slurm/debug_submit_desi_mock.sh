...
for (( i = START_INDEX; i <= END_INDEX; i += STEP )); do
    ...
    srun -N 1 -n 1 -c 8 --output="debug_mock_run_${LEVEL2_START}-${LEVEL2_END}_%j_%t.log" --error="debug_error_mock_${LEVEL2_START}-${LEVEL2_END}_%j_%t.log" python desi-DLAGP.py \
        --qsocat "$QSOCAT" \
        --release "$RELEASE" \
        --program "$PROGRAM" \
        --survey "$SURVEY" \
