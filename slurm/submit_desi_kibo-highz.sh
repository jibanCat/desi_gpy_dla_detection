...
SURVEY="${SURVEY:-main}"
OUTDIR="${OUTDIR:-/pscratch/sd/j/jibancat/desi-kibo-gpdla-highz/}"
...
for (( i = HPX_START_INDEX; i <= HPX_END_INDEX; i += HPX_STEP )); do
    ...
    srun -N 1 -n 1 -c 32 --output="high-z_kibo_run_${HPX_START}-${HPX_END}_%j_%t.log" --error="error_high-z_kibo_${HPX_START}-${HPX_END}_%j_%t.log" python desi-DLAGP-highz.py \
        --qsocat "$QSOCAT" \
        --release "$RELEASE" \
        --program "$PROGRAM" \
        --survey "$SURVEY" \
