...
HPX_END_INDEX="${HPX_END_INDEX:-1426}"  # 46 healpix pixels * 32 tasks = 1472 healpix pixels
...
for (( i = HPX_START_INDEX; i <= HPX_END_INDEX; i += HPX_STEP )); do
    ...
    echo "Running for missing healpix ${HPX_START} <= HPX < ${HPX_END}"

    srun -N 1 -n 1 -c 8 --output="patch_kibo_run_${HPX_START}-${HPX_END}_%j_%t.log" --error="patch_error_kibo_${HPX_START}-${HPX_END}_%j_%t.log" python desi-DLAGP.py \
        --qsocat "$QSOCAT" \
        --release "$RELEASE" \
        --program "$PROGRAM" \
        --survey "$SURVEY" \
        ...
        --hpx_start "$HPX_START" \
        --hpx_end "$HPX_END" \
        --external_hpx_list "missing_hpx_list.txt" & 
done
