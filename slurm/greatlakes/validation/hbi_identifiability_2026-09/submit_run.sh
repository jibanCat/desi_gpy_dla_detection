#!/bin/bash
# submit_run.sh RUN_ID "[EXTRA cc_real_posterior flags]" [OUT_SUBDIR]
...
ROOT=${ROOT:-/scratch/cavestru_root/cavestru0/mfho/hbi_validation_2026-09-02}
...
m = dict(run_id="$RUN_ID", out_dir="$OUT", timestamp=datetime.datetime.now().astimezone().isoformat(),
         ...
         parent_run=("frozen POOLED_ln_real_v2_20260821 (ea881b5f…)" if "$RUN_ID"=="R0" else "R0"),
         jobs={})
