...
#SBATCH --job-name=preload    # Job name for identification in the queue
#SBATCH --output=preload_loa_%j.log  # Standard output log (%j is replaced by the job ID)
...
for (( i = HPX_START_INDEX; i <= HPX_END_INDEX; i += HPX_STEP )); do
    ...
    srun -N 1 -n 1 -c 8 --output="loa_run_${HPX_START}-${HPX_END}_%j_%t.log" --error="error_loa_${HPX_START}-${HPX_END}_%j_%t.log" python preload_spectra/desi-preload.py \
        --qsocat "$QSOCAT" \
        --release "$RELEASE" \
        --program "$PROGRAM" \
        --survey "$SURVEY" \
