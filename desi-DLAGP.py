...
def main(args=None):
    ...
    if args.mocks:
        ...
        if args.pixel_col == "UNIQPIX":
            # The UNIQPIX/matterhorn coadd-path dispatch was deliberately
            # deferred to a separate PR (98b50a4: the dlasearch findfile call
            # is incompatible with desispec 0.70.0; PI decision "ship
            # separately"). Fail LOUDLY rather than run a half-dispatch that
            # groups by UNIQPIX but resolves legacy healpix coadd paths.
            log.error(
                "--pixel_col UNIQPIX is not available in this frozen state: "
                "the matterhorn coadd-path dispatch ships separately "
