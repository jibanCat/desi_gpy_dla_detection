...
def write_readme(scratch_dir: Path):
    ...
    h5_here = scratch_dir / "phase2_result.h5"
    npz_here = scratch_dir / "phase2_result.npz"
    if h5_here.exists():
        body_lines.append(f"- `phase2_result.h5` — **final trained model** (mirrored from repo `out_dir`; copy this file to NERSC for inference).")
    if npz_here.exists():
        body_lines.append(f"- `phase2_result.npz` — training-history record (loss / log_*_history per iter).")
    body_lines.append("")
    body_lines.append("## Authoritative model artifact")
    body_lines.append("")
    if h5_here.exists():
        body_lines.append(
            f"**Local copy at `phase2_result.h5`** (this dir). "
            f"Production-loadable by `gpy_dla_detection.null_gp.NullGPMAT`."
        )
        body_lines.append("")
        body_lines.append(
            f"Authoritative repo copy: `{out_dir}/phase2_result.h5`. The two are "
            "byte-identical at write time (trainer copy2's to both; see "
            "`tests/phase2_train_desi.py:716-731`)."
        )
    elif cfg.get("out_dir"):
        body_lines.append(
            f"The trained model lives at `{out_dir}/phase2_result.h5` (in repo). "
            f"For older runs the trainer didn't yet mirror to scratch; "
            f"`examples/write_scratch_readmes.py` is what just put this README here, "
            "and the same script can be re-run after manually copying the .h5."
        )
    else:
    ...
    body_lines.append(
        ...
        "Use `phase2_result.h5` (in this dir if mirrored, else in repo `out_dir`)."
    )
    body_lines.append("")
