#!/usr/bin/env python
"""Lyα-only window + P_DLA>0.9999 variant of the raw CDDF/dN/dX/Ω comparison.

Two changes vs the canonical cddf_snr_bal_consistent.py:
  1. Lyα-only window: restrict the DLA redshift to the Lyα forest (blueward edge
     at the QSO Lyβ EMISSION line): z_dla > (λ_Lyβ/λ_Lyα)·(1+z_qso) − 1, applied
     to BOTH the detections (numerator) and the path ΔX (denominator).
  2. P_DLA > 0.9999 (stricter purity) instead of 0.99.

Everything else identical: DLAFLAG==0 (Lyβ-veto + BAL + inference flags), SNR>2 &
non-BAL applied to numerator AND ΔX, raw (no completeness correction). Per-
sightline z_qso/min_z/max_z/snr/tid from processed-*.h5 (cached v3, adds z_qso).
"""
...
LYA = 1215.6701; LYB = 1025.7223; R = LYB / LYA          # 0.84375  Lyα-forest blue edge
PDLA_MIN = 0.9999
...
def parent(procdir, tag, nproc):
    ...
    cf = os.path.join(CACHE, tag.split()[0] + "_parent_v3.npz")   # v3 = adds z_qso
    if os.path.exists(cf):
        z = np.load(cf); print(f"[{tag}] cache {cf}", flush=True)
        return z["tid"], z["zq"], z["mn"], z["mx"], z["snr"]
    ...
    zq = np.concatenate([x[1] for x in r]).astype(float)
    ...
    tid, zq, mn, mx, sn = tid[u], zq[u], mn[u], mx[u], sn[u]
    g = np.isfinite(mn) & np.isfinite(mx) & np.isfinite(sn) & np.isfinite(zq) & (mx > mn)
    tid, zq, mn, mx, sn = tid[g], zq[g], mn[g], mx[g], sn[g]
    np.savez(cf, tid=tid, zq=zq, mn=mn, mx=mx, snr=sn)
    return tid, zq, mn, mx, sn

def load_abs(fits):
    d = fitsio.read(fits, ext=1, columns=["DLAFLAG", "SNR_REDSIDE", "P_DLA", "NHI", "Z_DLA", "Z_QSO"])
    zq = np.asarray(d["Z_QSO"], float); zd = np.asarray(d["Z_DLA"], float)
    lya_only = zd > (R * (1 + zq) - 1)                 # Lyα-forest only
    k = (np.asarray(d["DLAFLAG"]) == 0) & (np.asarray(d["P_DLA"], float) > PDLA_MIN) & lya_only
    return zd[k], np.asarray(d["NHI"], float)[k], np.asarray(d["SNR_REDSIDE"], float)[k]
...
def main():
    ...
    for label, color, fits, procdir, balcat in CFG:
        tid, zq, mn, mx, sn = parent(procdir, label, a.nproc)
        bal = np.unique(fitsio.read(balcat, ext=1, columns=["TARGETID"])["TARGETID"].astype(np.int64))
        not_bal = ~np.isin(tid, bal)
        # Lyα-only path window: raise blue edge to the QSO Lyβ emission
        wlo = np.maximum(mn, R * (1 + zq) - 1); whi = mx
        valid = whi > wlo
        Xc = AbsorptionDistance(zmax=float(mx.max()), Omega_m=0.279)
        hi = (sn > 2) & not_bal & valid
        nb = not_bal & valid
        Xhi = total_DeltaX_in_zbins(zbins, wlo[hi], whi[hi], Xc)
        Xall = total_DeltaX_in_zbins(zbins, wlo[nb], whi[nb], Xc)
        ...
        print(f"[{label}] BAL={100*(1-not_bal.mean()):.1f}% | Lyα-only & SNR>2 & nonBAL path frac of (nonBAL Lyα)"
              f"={Xhi.sum()/Xall.sum():.3f} | dets(flag0&P>{PDLA_MIN}&Lyα)={len(z)}", flush=True)
    loa, mh = D

    def dndx(d, thr, sel):
        X = d["Xhi"] if sel == "hi" else d["Xall"]
        m = (d["nhi"] >= thr) & ((d["snr"] > 2) if sel == "hi" else True)
    ...
    ax.set_title("Raw dN/dX (NHI ≥ 20.3, Lyα-only, P_DLA>0.9999, non-BAL)\nLOA & matterhorn vs N12/PW09", fontsize=11)
    ax.legend(fontsize=8); ax.grid(alpha=.3)
    fig.tight_layout(); fig.savefig(os.path.join(OUT, "dndx_lya_p9999.png"), dpi=140); plt.close(fig)

    # 2. CDDF
    def cddf(d, sel):
        X = d["Xhi_full"] if sel == "hi" else d["Xall_full"]
        m = (d["z"] >= 2.0) & (d["z"] < 4.4) & (d["nhi"] >= 20.0) & ((d["snr"] > 2) if sel == "hi" else True)
    ...
    ax.set_title("Raw CDDF (NHI ≥ 20, Lyα-only, P_DLA>0.9999, non-BAL)\nLOA & matterhorn vs N12/Ho21", fontsize=11)
    ax.legend(fontsize=8); ax.grid(alpha=.3, which="both")
    fig.tight_layout(); fig.savefig(os.path.join(OUT, "cddf_lya_p9999.png"), dpi=140); plt.close(fig)

    # 3. Omega
    sel_n = (logN_mid >= 20.3) & (logN_mid <= 22.0)
    def omega(d, sel):
        X = d["Xhi"] if sel == "hi" else d["Xall"]
        m = (d["snr"] > 2) if sel == "hi" else np.ones(len(d["z"]), bool)
    ...
    ax.set_title("Raw Ω_HI(z) (Lyα-only, P_DLA>0.9999, non-BAL)\nLOA & matterhorn vs literature", fontsize=11)
    ax.legend(fontsize=8, ncol=2); ax.grid(alpha=.3)
    fig.tight_layout(); fig.savefig(os.path.join(OUT, "omega_lya_p9999.png"), dpi=140); plt.close(fig)

    with open(os.path.join(OUT, "lya_p9999_summary.txt"), "w") as f:
        f.write("Lyα-only window + P_DLA>0.9999 + SNR>2 & non-BAL (raw, no completeness corr.)\n")
        for d in D:
            f.write(f"{d['label']}: ΔX(Lyα,nonBAL,SNR>2)={d['Xhi_full']:.4g}  dets={len(d['z'])}\n")
            _, dh, _ = dndx(d, 20.3, "hi")
            f.write("   dN/dX(NHI>=20.3, SNR>2) per z: " + " ".join(f"{v:.4f}" for v in dh) + "\n")
    print("[done] wrote Lyα/P9999 plots + summary to", OUT, flush=True)
