...
canonical contract (the same one the high-z catalogue and R-039 use):
  ...
  candidate with |dz| / (1 + z_true) < 0.01 (the canonical matcher tolerance; MAX_DLAS = 1
  returns at most one candidate per sightline so the match is unique).
...
a pair are matched; outcomes zero / one / two candidates (MAX_DLAS = 1 -> at most one),
which absorber wins, merged-N-hat bias, threshold migration.
"""
...
def main():
    ...
    for tpath, odir in zip(a.truth, a.outputs):
        ...
        for t in truth:
            ...
            rec = dict(TARGETID=tid, wave=int(t["wave"]), inj_idx=int(t["inj_idx"]), logN=float(t["logN"]), z_inj=zt, stratum=int(t["stratum"]),
                       ...
                       any_accepted=(len(cands) > 0), accepted_nhat=(float(cands[0]["NHI"]) if cands else np.nan), accepted_z=(float(cands[0]["Z_DLA"]) if cands else np.nan),
    ...
    if pairs:
        ...
        for cls in sorted({p["pair_class"] for p in pair_tab}):
            ...
            summ[cls] = dict(n_pairs=len(ps), frac_zero=float(np.mean([p["n_accepted"] == 0 for p in ps])), frac_one=float(np.mean([p["n_accepted"] == 1 for p in ps])),
                             frac_two=0.0, frac_winner_is_higher_N=float(np.mean([p["winner"] == int(np.argmax(p["logN"])) for p in ps if p["n_accepted"]])) if any(p["n_accepted"] for p in ps) else None,
    ...
    for c in tables["per_logN_point_all"]:
        print(f"  logN {c['key']['logN']:.2f}: n {c['n']:4d} C {c['C']:.3f} [{c['C_lo68']:.3f},{c['C_hi68']:.3f}] rep20.3 {c['reporting_C_T20.3']} dlogN {c['dlogN_mean']} +- {c['dlogN_std']}")
    if pairs:
