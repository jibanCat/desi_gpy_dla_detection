...
from .null_meanflux_gp import NullMFGP
...
def process_sample(
    ...
    params: Parameters,
    sample_log_likelihood_k_dlas: Callable[[int, np.ndarray, np.ndarray], float],
    ...
    log_likelihood = sample_log_likelihood_k_dlas(i, z_dlas, nhis) - np.log(
...
class DLAMFGP(NullMFGP):
    ...
    def sample_log_likelihood_k_dlas(
        self, i: int, z_dlas: np.ndarray, nhis: np.ndarray
    ) -> float:
        """
        Compute the log likelihood of k DLAs within a quasar spectrum:
            p(y | λ, σ², M, ω, c₀, τ₀, β, τ_kim, β_kim, {z_dla, logNHI}_{i=1}^k)

        :param i: the index of the sample. This is used to query the Meanflux samples.
        ...
        dla_mu, dla_M, dla_omega2 = self.this_dla_gp(i, z_dlas, nhis)
    ...
    def this_dla_gp(
        ...
        # Marginalize over tau_0 and beta, and these are precomputed in the NullGP
        dla_mu = self.precomputed_mu[i] * absorption
        dla_M = self.precomputed_M[i] * absorption[:, None]
        dla_omega2 = self.precomputed_omega2[i] * absorption**2
...
class DLAMFGPMAT(DLAMFGP):
    ...
    def __init__(
        ...
        broadening: bool = True,
        prev_tau_0: float = 0.0023,  # Not used in the current implementation
        prev_beta: float = 3.65,  # Not used in the current implementation
    ):
