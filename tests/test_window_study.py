...
def test_committed_artifact_carries_NO_fabricated_ratification(ARTIFACT, WS):
    ...
    chi2_dof_max: 3}`; that field must be gone everywhere in the tree.

    The rule is the widened import-time guard's: ANY field whose NAME contains
    "ratified" must be a collection holding ONLY allow-listed items. A scalar
    under such a name fails too — a bool or a count called `..._ratified_...`
    is unauditable, and the guard cannot tell it apart from a claim.
    """
    allowed = set(WS.PI_RATIFIED_ITEMS)
    bad = []
    for path, key, val in _walk_keys(ARTIFACT):
        if key in ("ratified_arms", "gate_tolerances_ratified"):
            bad.append(f"{path}  <- the RETRACTED field name")
        elif "ratified" in key.lower():
            if not isinstance(val, (dict, list)):
                bad.append(f"{path}  <- scalar {val!r} under a 'ratified' name")
            else:
                stray = set(val) - allowed
                if stray:
                    bad.append(f"{path} -> {sorted(stray)}")
    assert not bad, bad
    # ... and the allow-list must actually be published, or the rule is vacuous
    assert ARTIFACT["metadata"]["gate"]["pi_ratified_items"] == \
        list(WS.PI_RATIFIED_ITEMS)
